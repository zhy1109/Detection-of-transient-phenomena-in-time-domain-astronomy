...
test_data_path = os.path.join(test_data_path, "test1.npy")
...
for i in range(util.test_end_id - util.test_end_id):
	error = np.square(np.subtract(test_data[i, ..., 0], reconstructed_data[i, ..., 0]))
...
anomaly_pos = np.zeros(1187)
root_cause_gt = np.zeros((1187, 3))
anomaly_span = [5, 10, 15]
...
root_cause_f=pandas.read_csv('../data/test_anomaly1.csv')
# root_cause_gt = np.loadtxt(root_cause_f, delimiter=",", dtype=np.int32)
# anomaly_pos = root_cause_gt[:, 0]
root_cause_gt=root_cause_f
anomaly_pos = [(anomaly_pos[i]/util.gap_time-util.test_start_id-anomaly_span[i % 3]/util.gap_time) for i in range(1187)]
for i in range(util.test_end_id - util.test_end_id):
